...
func (gf *GGUFFile) transformerArchitecture(arch string) (ga GGUFArchitecture) {
	...
	case "exaone4":
		// Only the 64-block EXAONE 4.0 interleaves windows,
		// see https://github.com/ggml-org/llama.cpp/blob/16d222fc5f2c0fdc3d0180e0b772516ec6e2eddd/src/models/exaone4.cpp#L8-L14.
		if ga.BlockCount == 64 {
			if ga.AttentionSlidingWindow == 0 {
	...
	if v, ok := m[attentionSlidingWindowPatternKey]; ok {
		...
		if v.ValueType == GGUFMetadataValueTypeArray {
			av := v.ValueArray()
			ga.AttentionSlidingWindowLayers = make([]bool, av.Len)
	...
	if len(ga.AttentionSlidingWindowLayers) == 0 && swaPeriod != 1 && ga.AttentionSlidingWindow > 0 && ga.BlockCount > 0 {
	...
	if (arch == "lfm2" || arch == "lfm2moe") && ga.AttentionSlidingWindow > 0 {
		ga.AttentionSlidingWindowLayers = make([]bool, len(ga.AttentionHeadCountKVs))
		for i, headsKV := range ga.AttentionHeadCountKVs {
			ga.AttentionSlidingWindowLayers[i] = headsKV > 0
		}
	}
	...
	if v, ok := m[attentionSharedKVLayersKey]; ok && ga.BlockCount > 0 {
		...
		if shared := ValueNumeric[uint64](v); shared <= ga.BlockCount {
